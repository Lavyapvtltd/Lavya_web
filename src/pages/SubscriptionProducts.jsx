...
const SubscriptionProducts = () => {
    ...
    const orders = useSelector(state => state.orders.subscription_orders);
                                        ...
                                        <div className="col-lg-12 col-md-12 col-12" key={index}>
                                            <div className="d-flex align-items-center border rounded-1 p-3">
                                                ...
                                                <div className="ps-3 w-100">
                                                    ...
                                                    <p className='my-3'><span className="quantity mt-1 fw-semibold">{`${order.deliveryType} Qty: ${order?.product[0]?.selQty}`}</span></p>
                                                    ...
                                                    <div className='w-100 d-flex justify-content-between'>
                                                        <p className="mt-1 fw-semibold d-flex align-items-center">Price: <span className='px-1'><i class="fa fa-inr" aria-hidden="true"></i> {order?.product[0]?.price}</span></p>
                                                        {
                                                            order.status != "CANCELLEDBYCUSTOMER" &&
                                                            <>
                                                                <div className='mt-1'>
                                                                    <button className="prim_color_bg py-2 px-3 text-white btn-effect-1" style={{ fontSize: "14px" }} onClick={()=>handleEdit(order)}>Edit</button>
                                                                </div>
                                                            </>
                                                        }
                                                    </div>
                                                    <p class="d-flex align-items-center text-secondary">You are saving  <span class="currency-symbol text-secondary px-1"><i class="fa fa-inr" aria-hidden="true"></i></span> {(order?.product[0]?.regularPrice - order?.product[0]?.price).toFixed(2)}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ))
